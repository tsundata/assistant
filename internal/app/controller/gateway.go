...
type GatewayController struct {
	...
	middleSvc  service.MiddleSvcClient
	userSvc    service.UserSvcClient
	chatbotSvc service.ChatbotSvcClient
...
func NewGatewayController(
	...
	chatbotSvc service.ChatbotSvcClient,
	userSvc service.UserSvcClient,
	storageSvc pb.StorageSvcClient) *GatewayController {
...
func (gc *GatewayController) App(c *fiber.Ctx) error {
	...
	reply, err := gc.middleSvc.GetCredential(md.BuildAuthContext(enum.SuperUserID), &pb.CredentialRequest{Type: category})
	if err != nil {
		return err
	}
	provider := vendors.NewOAuthProvider(gc.rdb, category, gc.config.Gateway.Url)
	return provider.Redirect(c, reply)
}
...
func (gc *GatewayController) OAuth(c *fiber.Ctx) error {
	...
	provider := vendors.NewOAuthProvider(gc.rdb, category, gc.config.Gateway.Url)
	app, err := provider.StoreAccessToken(c, reply)
	if err != nil {
		gc.logger.Error(err)
		return c.Status(http.StatusBadRequest).SendString(err.Error())
	}
	appReply, err := gc.middleSvc.StoreAppOAuth(md.BuildAuthContext(enum.SuperUserID), &pb.AppRequest{
		App: app,
	})
	if err != nil {
		return err
